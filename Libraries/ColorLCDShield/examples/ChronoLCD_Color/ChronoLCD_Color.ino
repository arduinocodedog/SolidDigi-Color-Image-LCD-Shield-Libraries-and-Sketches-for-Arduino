...
void displayDigitalTime(int h, int m, int s, int ap)
{
  ...
  /* add some blank spaces after the time, otherwise it'll display
     ...
     No, don't add blank spaces, you need to NULL-terminate the string, and the
     array needed to be larger too ... the array size should have been at least 13. 
     It kept resetting the Arduino so the clock reset and it wasn't keeping time at all!
  */
  timeChar[11] = NULL;
...
void drawClock()
{
  ...
  lcd.setStr("12", CLOCK_CENTER - CLOCK_RADIUS + 2, 66-9, C_COLOR, BACKGROUND);
